...
LeftCameraWindows::LeftCameraWindows(QWidget *parent, Qt::WindowFlags f)
    :QWidget(parent,f)
{
//    this->hide();
//    this->setAttribute(Qt::WA_TranslucentBackground);
    std::string filename = "/data/user/caliberation_result/left/result.xml";
   ...
   QFile file("/data/user/point/left/point.txt");
...
void LeftCameraWindows::readFrame(int frameOffset)
{
...
void LeftCameraWindows::my_show_fun()
{
...
void LeftCameraWindows::Start_warpPerspective()
{
    cv::Mat src;
    src = sourcePicture;
    ...
    if (initWarpPerspective)
    {
        ...
        dstTri[0] = cv::Point2f(0,720);
        dstTri[1] = cv::Point2f(0,0);
        dstTri[2] = cv::Point2f(98,135);
        dstTri[3] = cv::Point2f(98,585);

       //dstTri[0] = cv::Point2f(0,555);
        //dstTri[1] = cv::Point2f(120,555);
        //dstTri[2] = cv::Point2f(0,165);
        //dstTri[3] = cv::Point2f(120,165);
        ...
        for (int i=0;i<warp_dst.rows;i++)
        {
            ...
            {
                startX[i] = 0;
                endX[i] =120 - (((float)(120)/165) * (i-555));
...
//
void LeftCameraWindows::testLoopupTable()
{
    cv::Mat src;
    ...
    for(int i=0,k=0;i<warp_dst.rows;i++)
    {
        for(int j=0;j<warp_dst.cols;j++)
        {
            ...
            if ( (j>=startX[i]) && (j<=endX[i]) )
            {
                ...
                tempTablePos = caleLoopupTablePos(0,(UTL1Array[ (array[k].y * mapx.cols) + (array[k].x) ].x),( UTL1Array[ (array[k].y * mapx.cols) + (array[k].x) ].y),2);
...
void LeftCameraWindows::use_fisheye_show()
{
    uchar* srcdata;
        ...
        //if ( (j>=dstTri[0].y) && (j<=dstTri[1].y) )
        {
            ...
            for (int i=0; i<nc; i++) {
                ...
                if ( (i>=startX[j]) && (i<=endX[j]) )
                {
                    ...
                    srcdata += (((tempTablePos.y)-(srcheight * 2))*srcWidth*lenght);
...
void LeftCameraWindows::CaptureView(int frameOffset)
{
...
void LeftCameraWindows::initUTL1Array()
{
...
QImage LeftCameraWindows::cvMat2QImage(const cv::Mat& mat)
{
...
cv::Mat& LeftCameraWindows::ScanImageAndReduceC(cv::Mat& I, const uchar* const table)
{
...
QImage * LeftCameraWindows::getFinishImage()
{
    return finishImg;
}

QImage * LeftCameraWindows::getFisheyeImage()
{
    return image;
}

LeftCameraWindows::~LeftCameraWindows()
{
    free(cameraBuffer);
    for (int i=0;i<bufferCount;i++)
    {
