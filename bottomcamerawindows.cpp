...
BottomCameraWindows::BottomCameraWindows(QWidget *parent, Qt::WindowFlags f)
    :QWidget(parent,f)
{

//    this->hide();
//    this->setAttribute(Qt::WA_TranslucentBackground);
    std::string filename = "/data/user/caliberation_result/bottom/result.xml";
    ...
    QFile file("/data/user/point/bottom/point.txt");
    ...
    camera = new CameraWorkBottom();
    camera->setCameraName("/dev/video1");
    camera->setCameraData(&cameraBuffer);
    connect(camera,SIGNAL(whichBottomFrame(int)),this,SLOT(readFrame(int)));
    //camera->start();

//    camera.setCameraName("/dev/video2");
//    camera.setCameraData(&cameraBuffer);
//    camera.start();
...
void BottomCameraWindows::readFrame(int frameOffset)
{
...
void BottomCameraWindows::Start_warpPerspective()
{
    //QTime time;
    //time.start();
    ...
    if (initWarpPerspective)
    {
        ...
        dstTri[0] = cv::Point2f(420, 165);
        dstTri[1] = cv::Point2f(0,165 );
        dstTri[2] = cv::Point2f( 98,30 );
        dstTri[3] = cv::Point2f( 322,30);

        //dstTri[0] = cv::Point2f( 120,165 );
        //dstTri[1] = cv::Point2f( 0,0 );
        //dstTri[2] = cv::Point2f( 300,165 );
        //dstTri[3] = cv::Point2f( 420,0);
...
//
void BottomCameraWindows::testLoopupTable()
{
    cv::Mat src;
    ...
    for(int i=0,k=0;i<warp_dst.rows;i++)
    {
        for(int j=0;j<warp_dst.cols;j++)
        {
            ...
            if ( (j>=startX[i]) && (j<=endX[i]) )
            {
                ...
                tempTablePos = caleLoopupTablePos(0,(UTL1Array[ (array[k].y * mapx.cols) + (array[k].x) ].x),( UTL1Array[ (array[k].y * mapx.cols) + (array[k].x) ].y),1);
                writeLoopTablePos(i + 555,j,tempTablePos);
            }
            k++;
        }
    }
...
void BottomCameraWindows::use_fisheye_show()
{
    ...
    int lenght = 4;
        ...
        //if ( (j>=dstTri[0].y) && (j<=dstTri[1].y) )
        {
            ...
            for (int i=0; i<nc; i++) {
                ...
                if ( (i>=startX[j]) && (i<=endX[j]) )
                {
                    tempTablePos = readLoopupTablePos(j+555,i);

                    srcdata = (uchar *)(g_g2d_dst_buffer[frameCount]->buf_vaddr);
                    srcdata += (((tempTablePos.y)- (srcheight * 1))*srcWidth*lenght);
...
void BottomCameraWindows::CaptureView(int frameOffset)
{    
   ...
   //QTime time;
    ...
    //cv::Mat rgb;
        ...
        uchar * srcData = (uchar *)(g_g2d_dst_buffer[frameOffset]->buf_vaddr);
...
void BottomCameraWindows::initUTL1Array()
{
...
void BottomCameraWindows::my_show_fun()
{
...
QImage BottomCameraWindows::mat_to_qimage_ref(cv::Mat &mat, QImage::Format format)
{
    //return QImage(mat.data, mat.cols, mat.rows, mat.step, format);
}

QImage BottomCameraWindows::cvMat2QImage(const cv::Mat& mat)
{
...
cv::Mat& BottomCameraWindows::ScanImageAndReduceC(cv::Mat& I, const uchar* const table)
{
...
QImage *  BottomCameraWindows::getFinishImage()
{
    return finishImg;
}

QImage * BottomCameraWindows::getFisheyeImage()
{
    return image;
}

BottomCameraWindows::~BottomCameraWindows()
{
    free(cameraBuffer);
    for (int i=0;i<bufferCount;i++)
    {
